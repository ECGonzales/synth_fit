...
# Stephanie Douglas
# Modifed by Eileen Gonzales to just use for a chi-squared calculation.
################################################################################
...
import pickle
# from synth_fit.smooth import *  # I will not be smoothing in the code.
...
sub_cmap = LinearSegmentedColormap.from_list('trunc({n},{a:.2f},{b:.2f})'.format(n=cmap.name, a=0.2, b=1),
                                             cmap(np.linspace(0.2, 1, 6)))
...
def test_all(data_wave, data_flux, data_unc, model_dict, params,
    ...
    plt.savefig(
        '/Users/eileengonzales/Dropbox/BDNYC/BDNYC_Research/Python/Modules/synth_fit/Run_outputs/Figures/Teff_goodnesses_{}'.format(
            shortname) + '.pdf')
    plt.clf()

    fb = open(
        '/Users/eileengonzales/Dropbox/BDNYC/BDNYC_Research/Python/Modules/synth_fit/Run_outputs/chisquares_{}'.format(
            shortname) + '.pkl', 'wb')
